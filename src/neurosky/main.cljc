(ns neurosky.main
  "Kotodama WASM entrypoint for the Neurosky clean-room actor (L4) — Clojure port.
  ...
  facts (`neurosky.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "neurosky")
(def tier "L4")
...
(def entity-specs
  [{:entity "Device"      :plural "devices"      :id-prefix "neurosky_dev"
    :fields [:model :channels :samplingHz :firmware]        :required [:model :channels]
    :coerce {:channels :int :samplingHz :int}          :refs {}}
   {:entity "Session"     :plural "sessions"     :id-prefix "neurosky_ses"
...
(defn emit-facts
  "EAVT facts for one record: {\"neurosky.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "neurosky-compat" :tier tier :entities entities} 200])
...
(defn start! [] :neurosky-compat/ready)
